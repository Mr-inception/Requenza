...
const QuizQuestion = ({
  ...
  return (
    <div className="min-h-screen w-full bg-slate-900 text-white flex flex-col px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-12 space-y-6 sm:space-y-8 lg:space-y-10 animate-slide-in">
      {/* Progress */}
        ...
        {question.type === 'single' ? (
          ...
          >
            {question.options.map((option) => (
              <div key={option.id} className="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 lg:p-6 rounded-lg border border-slate-700 hover:border-primary/50 transition-colors bg-slate-800/50">
                <RadioGroupItem value={option.id} id={option.id} className="shrink-0 mt-1" />
                <Label htmlFor={option.id} className="flex-1 cursor-pointer">
                  <div className="font-medium text-base sm:text-lg lg:text-xl">{option.label}</div>
                  {option.description && (
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 lg:gap-6 justify-between pt-4 sm:pt-6 lg:pt-8">
        <Button
          variant="outline"
          className="w-full sm:w-auto bg-slate-700 text-white hover:bg-slate-600 border-slate-700 text-sm sm:text-base lg:text-lg py-3 sm:py-4 lg:py-5"
          onClick={onPrevious}
        ...
        <Button
          onClick={onNext}
          disabled={!canGoNext}
          className="w-full sm:w-auto bg-primary hover:bg-primary-glow text-sm sm:text-base lg:text-lg py-3 sm:py-4 lg:py-5"
        >
